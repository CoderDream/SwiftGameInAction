...
class GameViewController: UIViewController {
    ...
    var completeVC: UIViewController = UIViewController()
    ...
    @IBAction func fireBtnClick(_ sender: UIButton) {
        ...
        //Timer.scheduledTimer(timeInterval: 0.003, target: self, selector: #selector(PlayerObject.fireBullet), userInfo: nil, repeats: true)
    }
...
//            }
    ...
    @objc func hideEnemy() {
        ...
        if image?.isHidden == false {
            j += 10
            scoreLabel.text = "\(j)"
            Timer.scheduledTimer(timeInterval: 0.0002, target: self, selector: #selector(complete), userInfo: nil, repeats: false)
            image?.isHidden = true
    ...
    @objc func complete() {
        ...
        if j >= 50 {
            self.changeState(newState: .RELOADING)
            completeVC = self.storyboard!.instantiateViewController(withIdentifier: "completevc") as UIViewController
            self.view.addSubview(completeVC.view)
        }
    }
