...
class PlayerBullet: NSObject {
    ...
    func fireBullet(gameView:UIView, playerView: UIImageView){
        ...
        imageRef1.frame = CGRect(x: 0, y: 0, width: 128, height: 128)
        let imageRef2 = UIImageView(image: bombImage1)
        imageRef2.frame = CGRect(x: 33, y: 0, width: 128, height: 128)
        ...
        self.bulletRect = CGRect(x: bulletStartX, y: playerView.frame.origin.y, width: 128, height: 128)
        ...
        // 移动子弹
        self.bulletTimer = Timer.scheduledTimer(timeInterval: 0.01, target: self, selector: #selector(moveBullet), userInfo: nil, repeats: true)
    }
